...
static int aston_net_probe(struct platform_device *pdev)
{
    int result=0;
...
static int aston_net_remove(struct platform_device *pdev)   //设备移除接口
{
...
static struct platform_device aston_net_device= {
    .name	= "aston_net",
    .id		= -1,
    .dev	= {
...
static int __init aston_net_init(void)
{
    ...
    platform_device_register(&aston_net_device);
...
static void __exit aston_net_cleanup(void)
{
    ...
    platform_device_unregister(&aston_net_device);
}
